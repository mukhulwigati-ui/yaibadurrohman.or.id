...
export default function Campaign({ initialData = [], mendesak = [], unggulan = [], pilihan = [] }: CampaignProps) {
      ...
      <div className="space-y-3.5 w-full text-left">
        ...
        <div className="space-y-3">
          {initialData.map((item) => {
            ...
            return (
              <Link key={item.id} href={`/campaign/${item.slug}`} className="group flex gap-3.5 items-center bg-white p-3.5 rounded-none border border-gray-200/90 shadow-sm hover:shadow-md transition block">
                <div className="w-28 sm:w-32 aspect-[16/10] bg-gray-100 rounded-none overflow-hidden shrink-0 shadow-inner">
                ...
                <div className="flex-1 space-y-1.5 py-0.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full bg-gray-100 h-1.5 rounded-none overflow-hidden">
                      <div className="bg-[#ff2e3b] h-full rounded-none" style={{ width: `${percentage}%` }} />
                    </div>
                  </div>
                </div>
              </Link>
  ...
  return (
    <div className="space-y-6 w-full text-left">
      ...
      {mendesak.length > 0 && (
        <section className="bg-white rounded-none p-4 sm:p-5 border border-gray-200/90 shadow-sm space-y-3.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1">
            {mendesak.map((item) => {
              ...
              const percentage = Math.min(Math.round((item.collectedRaw / target) * 100), 100);
              return (
                <Link key={item.id} href={`/campaign/${item.slug}`} className="group border border-gray-200/90 rounded-none overflow-hidden p-3 bg-gray-50/60 space-y-2.5 block hover:shadow-md transition">
                  <div className="aspect-[16/10] bg-gray-200 rounded-none overflow-hidden relative">
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                    {item.daysLeft && (
                      <span className="absolute top-2 left-2 bg-[#0d5c91] text-white text-[10px] font-bold px-2 py-0.5 rounded-none shadow">
        ...
        <section className="bg-white rounded-none p-4 sm:p-5 border border-gray-200/90 shadow-sm space-y-3.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1">
            {unggulan.map((item) => {
              ...
              return (
                <Link key={item.id} href={`/campaign/${item.slug}`} className="group border border-gray-200/90 rounded-none overflow-hidden p-3 bg-gray-50/60 space-y-2.5 block hover:shadow-md transition">
                  <div className="aspect-[16/10] bg-gray-200 rounded-none overflow-hidden">
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                  </div>
                  <h3 className="text-xs sm:text-sm font-bold text-slate-800 line-clamp-2 leading-snug group-hover:text-[#0d5c91] transition-colors">{item.title}</h3>
                  <div className="space-y-1">
          ...
          <div className="space-y-3.5 pt-1">
            {pilihan.map((item) => (
              <Link key={item.id} href={`/campaign/${item.slug}`} className="group flex gap-3.5 items-center border-b border-gray-100 pb-3.5 last:border-none block hover:opacity-90 transition">
                <div className="w-28 sm:w-32 aspect-[16/10] bg-gray-200 rounded-none overflow-hidden shrink-0 shadow-inner">
                  <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                </div>
                <div className="flex-1 space-y-1.5 py-0.5">
                  <h3 className="text-xs sm:text-sm font-bold text-slate-800 line-clamp-2 leading-snug group-hover:text-[#0d5c91] transition-colors">{item.title}</h3>
